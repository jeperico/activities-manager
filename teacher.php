...
<body>
    <header class="bg-blue text-white p-4 flex justify-between items-center">
        ...
        <form method="POST" action="">
            <button type="submit" name="logout" class="bg-red-500 px-4 py-2 rounded">Sair</button>
        </form>
        <?php
    ...
    <main class="w-main m-auto max-w-mainmx h-screen py-10">
        <div class="flex justify-end mb-4">
            <a href="./register_class.php" class="bg-green-500 text-white px-4 py-2 rounded">Cadastrar Turma</a>
        </div>
                    ...
                    <?php
                        ...
                        while ($row = mysqli_fetch_assoc($result)) {
                            ...
                            echo "<td class='py-2 text-center flex justify-center gap-2'>
                                <a href='visualizar_turma.php?id={$row['id_class']}' class='bg-yellow-500 text-white px-2 py-1 rounded'>Visualizar</a> ";
                                echo "<form method='POST' onsubmit='return confirm(\"Tem certeza que deseja excluir esta turma?\")'>";
                                    ...
                                    echo "<button type='submit' class='bg-red-500 text-white px-2 py-1 rounded'>Excluir</button>";
                                echo "</form>";
                            echo "</td>";
                            echo "</tr>";
                            $count++;
